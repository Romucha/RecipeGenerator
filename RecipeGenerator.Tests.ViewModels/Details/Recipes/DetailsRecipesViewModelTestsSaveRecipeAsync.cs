...
namespace RecipeGenerator.Tests.ViewModels.Details.Recipes
{
    ...
    {
        [Fact]
        public async Task SaveRecipeAsync_Normal()
        {
            var recipe = RecipeDataCollections.Normal[0];
            var viewModel = GetViewModel();
            var recipeId = recipe.Id;
            var alteredName = nameof(SaveRecipeAsync_Normal);

            await viewModel.GetRecipeAsync(recipeId);
            viewModel.Name = alteredName;
            await viewModel.SaveRecipeAsync();

            Assert.Equal(alteredName, viewModel.Name);
        ...
        public async Task SaveIngredientAsync_DoesNothing(int recipeId)
        {
